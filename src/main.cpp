...
#if defined(NDEBUG)
# error "Rascoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd6b3a24ee75321c7eeb26a64bf68304d1ad8c4e3dde0bc21cb904233981b0c7f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rascoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rascoin Signed Message:\n";
...
{
    // Rascoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 700000); // Rascoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Rascoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Rascoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rascoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rascoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x430cd570975114ff0884841ba9c674e230def7c1816d618cea8252f81d82e6e8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN.com 09/Dec/2019 Trump's former physician files to run for Congress in Texas";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 33 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b2d59408bff74466112316b4696f06eb49425fea580698ed5fb80937ab35b7b7b3721b9dbb7f19f2e0ae00382c8ccc6312dbd407502f94e41e383c7de0c0359f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1575942752;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087347004;

        if (fTestNet)
        {
            block.nTime    = 1575942611;
            block.nNonce   = 386293807;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa092e5cc554eaa67a9ddb40b1f10fecfc37f87f91313aebedb546ad7d2551dc3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xd1, 0xb7, 0xf2 }; // Rascoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RascoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RascoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RascoinMiner(CWallet *pwallet)
{
    printf("RascoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rascoin-miner");
    ...
    try { loop {
        ...
        printf("Running RascoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RascoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RascoinMiner, pwallet));
}
